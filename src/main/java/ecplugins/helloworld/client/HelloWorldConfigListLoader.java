...
public class HelloWorldConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final HelloWorldConfigList   m_configList;
    private final CgiRequestProxy m_cgiRequestProxy;
    private String                m_editorName;

    //~ Constructors -----------------------------------------------------------

    public HelloWorldConfigListLoader(
            HelloWorldConfigList   configList,
            Component       component,
            ...
            ChainedCallback callback)
    {
        ...
        m_cgiRequestProxy = new CgiRequestProxy(getPluginName(), "helloworld.cgi");
    }
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading HelloWorld configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading HelloWorld configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-HelloWorld/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("helloworldcfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for HelloWorld plugin '" + m_configPlugin + "'";
